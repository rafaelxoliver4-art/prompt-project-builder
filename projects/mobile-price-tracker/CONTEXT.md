...
> **Version:** 0.10.1 · **Last updated:** 2026-08-05 · Content decided by the Architect; written by Code.
...
- **Complete gap list — `history`: 44 snapshots over the 45 days from 2026-06-22 to 2026-08-05, MISSING exactly `2026-08-04` (98 % coverage). `convergent_history`: 2 of 3 days, missing the same date.** No other gap exists in either sheet.
...
- **THE LESSON, and it is a sharp one:** *a test that gates the scrape is a data-loss risk, not just a red X.* A CI test failure and a scrape failure have wildly different costs here, but the workflow treats them identically. The test was fixed (all three carriers stubbed, guard-agnostic) and the whole class of failure is now **detected** by F2/F3 above — but detection is not prevention. **Any test asserting a specific exit code or leaning on the sanity bands can kill a day of data if the world drifts.** New tests must be written with that in mind.
