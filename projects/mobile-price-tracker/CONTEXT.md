...
> **Version:** 0.12.1 · **Last updated:** 2026-08-05 · **Status: CLOSED OUT — steady-state operation.**
...
- **The bug (audit P1 #4).** `_merge_history` deduped on `(snapshot_date, carrier, state, _key)`. Claro's **controle** page reuses the **flex** slug: `claro:plano-flex-20gb` is a real **"Controle 20GB" R$44,90** *and* a real **"Flex 20GB" R$59,90**. The two collided and one was **silently dropped** — only 3 of 4 Claro Controle plans were ever stored. Fixed by adding `category` to the dedupe key. Live 2026-08-05: Claro control **3 → 4 rows**, R$44,90 present, nothing else lost or duplicated (all other carrier/category counts unchanged).
