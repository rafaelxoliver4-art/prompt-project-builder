...
> **Version:** 0.5.0 · **Last updated:** 2026-06-26 · Content decided by the Architect; written by Code.
...
- **TIM (`drupal-settings-json`):** there is **no clean structured validity field** — the period is in the oferta's benefits-modal HTML (`field_layout_canvas_segundo`: "...por 30 dias..." + shorter promo bonuses). `validity_days` = the **MAX "N dias"** in the oferta (the plan period dominates bonuses). All 3 XIP tiers are **30-day**: **R$20/6GB, R$25/8GB, R$30/16GB** → cheapest 30-day = **R$20/6GB** (not R$30). Fragile (HTML text, not a field) — documented; the raw capture is retained for re-parsing.
- **Matrix Pre column** = cheapest prepaid plan with `validity_days >= 28` (config `prepaid.min_validity_days`, mirrored in `excel_writer.PREPAID_MIN_VALIDITY_DAYS`), as a live `MINIFS` adding one criterion (`history!<validity_col>,">=28"`). Live Pre row (Jun-26): **TIM 20 / Vivo 30 / Claro 30** (R$/mo). Displayed as **monthly R$/mo** (default); the avg-R$/day form (JPMorgan-style = price ÷ validity) is a one-line switch if the Bridge prefers it.
...
> **Price-evolution matrix — MONTHLY, table-only (#16 monthly; #17 split, 2026-06-26; was daily #12):** the **`comparison`** sheet is the **Month × (category × carrier) matrix** (the Figure-5 form), now a **clean table** at the **TOP of the sheet** (group titles row 1, **TIM | Vivo | Claro** sub-header row 2, month rows from **row 3**; charts moved to a separate `Charts` sheet — see below). **Rows = month:** col A holds one row per distinct MONTH in `history` (built in Python by deduping `snapshot_date` to year-month via `evolution_months`), stored as the **first-of-month DATE** (`2026-06-01`) and display-formatted **`mmm-yy`** (→ Jun-26). Columns: `Month` + four groups (**Control / Post / Pre / Digital**, each merged over **TIM | Vivo | Claro**). Each value cell is a **live `_xlfn.MINIFS` formula over `history`** = the **cheapest R$ that carrier offered in that category DURING that month** — so the workbook stays **self-connected** and the matrix grows **one row per month** automatically (daily raw detail stays in the `history` sheet). **Digital** = `MIN(lite, flex)` via the reciprocal-max trick; 0 / no-match → blank so the **per-group heatmap** (green cheap → red dear) ignores it. **Pre** = cheapest recharge amount (true R$/day needs validity-days we don't capture — §8). Metric = **cheapest-in-month** (month-end value is an alternative if ever preferred). Same-day re-runs stay **idempotent per date** in `history`; the validated rank-aligned cross-section is preserved on the **`Ranking`** sheet. **Layout (#17):** only the **2 header rows + the Month column are frozen** (`freeze_panes="B3"`), so the table scrolls freely; `comparison` is the **active sheet on open** (the workbook lands on the table, not `history`); the sheet is **never protected** (`protection.sheet=False`).
>
> **⚠️ Formula gotcha — month scope is a MINIFS TEXT-PREFIX, not EOMONTH date bounds (verified in real Excel, #16):** `history.snapshot_date` is stored as **TEXT** (`'2026-06-22'`, not an Excel date serial). Excel **coerces any date/serial comparison criterion** — including `MINIFS(...,date,">="&$A2, date,"<="&_xlfn.EOMONTH($A2,0))` — to a number that **never matches a text column**, so EOMONTH/`>=`/`<=` month-range bounds return **blank** (empirically confirmed by recalc: all variants → 0/blank). The **only** construction that filters the text dates is a **prefix wildcard**: each cell matches `history.snapshot_date` against `YEAR($A<row>)&"-"&TEXT(MONTH($A<row>),"00")&"-*"` (i.e. all rows whose ISO date starts with the row's `YYYY-MM-`). The prefix is built from `YEAR()`/`MONTH()` + a **numeric `"00"`** format — NOT the `"yyyy"/"mm"` date codes, which **pt-BR Excel localizes** (year code is `"aaaa"`; `TEXT($A1,"yyyy-mm")` returns the literal `"yyyy-06"` here). So the month scope is locale-independent and computes correctly. `_minifs_month` in `excel_writer.py` carries this rationale. *(To use literal EOMONTH bounds we would have to store `history.snapshot_date` as real Excel dates — a `history`-data change this task explicitly excluded; flagged for the Architect.)*

> **Evolution charts — own `Charts` sheet (#13 → #16 monthly → #17 split, 2026-06-26):** the four per-category **line charts** (Control / Post / Pre / Digital) now live on a **dedicated `Charts` sheet** (2×2 layout, tab color `#43A047`), **no longer overlaying the matrix**. Each references the `comparison` matrix via **cross-sheet refs** (X = `'comparison'!$A$3:$A$<lastmonth>`, three series = the group's TIM/Vivo/Claro matrix columns), palette-matched lines (TIM `#0033A0`, Vivo `#660099`, Claro `#DA291C`), markers on, monthly X-axis, legend bottom. Refs are **rebuilt each run** to the months present; charts read the matrix (which reads `history`), so they **grow into trend lines** as monthly history accumulates. openpyxl native `LineChart` — no images/macros. **Charts populate on open in Excel** (which recalcs the matrix formulas); a populated GitHub *preview* would need a headless-recalc step in CI (out of scope — flag if wanted).
>
...
- **(i) Fidelity vs non-fidelity:** we capture the one displayed price; the cheapest can shift with loyalty (e.g. TIM Controle R$49,99 *fidelizado* vs R$58,99 *boleto*). Decide which to track — or capture both. **Still pending (#19).**
- **(ii) Consistent GB definition:** base vs base+bonus differs by carrier; needed for a fair data-tier view. **Still pending.**
- **(iii) Prepaid normalization:** ✅ **PARTLY IMPLEMENTED (#18):** each prepaid plan now carries `validity_days`, and the Pre column compares the cheapest **30-day** plan (validity_days >= 28) shown as monthly R$/mo (Claro R$1/dia → the real R$30/30d plan). The avg-R$/day form (JPMorgan-style = price ÷ validity) is a one-line switch if preferred. Remaining: a Vivo-Lite-style "digital prepaid" lens, and per-tier value comparison.
