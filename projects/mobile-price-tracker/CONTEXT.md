...
> **Version:** 0.12.0 · **Last updated:** 2026-08-06 · **Status: CLOSED OUT — steady-state operation.**
...
- **Complete gap list, as of 2026-08-06 — `history`: 44 snapshots over the 46 days from 2026-06-22, MISSING `2026-08-04` AND `2026-08-05` (96 % coverage). `convergent_history`: 2 of 4 days, missing the same two dates.** No other gap exists in either sheet.
- **Both days are UNRECOVERABLE.** Carrier pages show only *current* prices; there is nothing to re-read. They are **never** backfilled — history stays append-only and honest about the hole. *(Genuine 2026-08-05 raw captures do exist in the local, uncommitted `data/raw/`. Writing them under an 08-05 date would be recovery from a real archive rather than invention, but it is still backfilling a missing date, which the standing rule forbids — so it was NOT done. If the Bridge ever wants it, that is a decision for the Architect, not for Code.)*
