...
> **Price-evolution matrix — IMPLEMENTED (#12, 2026-06-22):** the **`comparison`** sheet is now a **Date × (category × carrier) matrix** — each value cell a **live `_xlfn.MINIFS` formula over the `history` sheet** (the cheapest R$ that carrier offered in that category on that date), so the workbook is **self-connected** and the matrix fills in automatically as daily history accumulates. Columns: `Date` + four groups (**Control / Post / Pre / Digital**, each merged over **TIM | Vivo | Claro**); cell metric = **cheapest**; **Digital** = `MIN(lite, flex)` via a reciprocal-max trick; 0 / no-match → blank so the **per-group heatmap** color-scale (green cheap → red dear) ignores it. **Pre** = cheapest **recharge amount** captured (true R$/day needs validity-days we don't yet capture — §8). **Next step:** a **monthly roll-up** (rows = month) to match the Figure-5 reference once enough days accumulate, plus event annotations from `changes`. Same-day re-runs are **idempotent per date** (a re-run replaces that date's rows, never unions). The validated rank-aligned cross-section is preserved on the **`Ranking`** sheet.

> **Evolution charts — IMPLEMENTED (#13, 2026-06-22):** four per-category **line charts** (Control / Post / Pre / Digital) in a **2×2 block at the top of the `comparison` sheet**, above the matrix. Each plots **price over time**: X-axis = the matrix Date column, three series = that group's TIM/Vivo/Claro matrix columns (palette-matched lines — TIM `#0033A0`, Vivo `#660099`, Claro `#DA291C`), markers on. The charts read from the matrix (which reads from `history`), so they **grow into trend lines automatically** as daily history accumulates; the data references are rebuilt each run to match the current date range. openpyxl native `LineChart` — no images/macros. **Charts populate on open in Excel** (which recalcs the matrix formulas); a populated GitHub *preview* would need a headless-recalc step in CI (out of scope — flag if wanted).
