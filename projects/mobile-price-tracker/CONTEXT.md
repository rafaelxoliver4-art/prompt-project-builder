...
> **Version:** 0.4.6 · **Last updated:** 2026-06-26 · Content decided by the Architect; written by Code.
...
> **Evolution charts — MONTHLY X-axis (#13 → #16, 2026-06-26):** four per-category **line charts** (Control / Post / Pre / Digital) in a **2×2 block at the top of the `comparison` sheet**, above the matrix. Each plots **cheapest R$ over time**: X-axis = the matrix **Month** column (`mmm-yy`), three series = that group's TIM/Vivo/Claro matrix columns (palette-matched lines — TIM `#0033A0`, Vivo `#660099`, Claro `#DA291C`), markers on. The charts read from the matrix (which reads from `history`), so they **grow into trend lines automatically** as monthly history accumulates; the data references are **rebuilt each run** to match the months present. openpyxl native `LineChart` — no images/macros. **Charts populate on open in Excel** (which recalcs the matrix formulas); a populated GitHub *preview* would need a headless-recalc step in CI (out of scope — flag if wanted).
