...
> **Version:** 0.13.0 · **Last updated:** 2026-08-05 · **Status: CLOSED OUT — steady-state operation.**
> Content decided by the Architect; written by Code.
...
- ⚠️ **REBUILT BY TIM ~2026-08-03 — parser rewritten in #36 (P0 fix for the #35 audit's blocker).** The section is now **2 regional blocks × 2 tier-versions × 2 plan types = 8 cards**, each linked to its own modal by `data-modal-open="modal-fit-<anual|mensal>-<version>-<regions>"` (e.g. `modal-fit-anual-2.0-sp-sc-rn-ce`, `…-demais-ufs`). Two bugs followed and together produced this project's only price alert, which was **FALSE**: a first-match regex captured a *different* newly-inserted "1.0" tier's price, and `_fit_code`'s exact `modal-fit-anual/mensal` lookup found nothing, so `plan_id` silently degraded `tim:TIM202600000271 → tim:fit-anual` (the #29 name fallback then manufactured the "move").
  - **Now driven by `data-modal-open`** — the authoritative card→modal link — so a card can only ever be paired with **its own** price, name and etiqueta.
  - **REGION GATE = the modal id's trailing region list** (`sp-sc-rn-ce` serves SP/SC/RN/CE; `demais-ufs` is the catch-all for every other state). **NOTE:** the Fit section carries **no `field_regioes`** — that field exists only on `ofertas` JSON nodes — so the region list is the only real gate the page offers (`rel=canonical` is state-less; DOM order is not a gate). A state named by no block falls through to `demais-ufs`; the parser never mixes blocks.
  - **plan_id = the etiqueta inside that card's own modal slice** — carrier-native, never price-derived, never a name slug. **SP tiers (live 2026-08-05): `tim:TIM202600000306` Anual 1.0 25GB 12x R$20 · `tim:TIM202600000271` Anual 2.0 35GB 12x R$30 · `tim:TIM202600000270` Mensal 1.0 20GB R$20 · `tim:TIM202600000305` Mensal 2.0 35GB R$45.**
  - **Loyalty:** `12x` on the card ⇒ the annual credit-card plan (`loyalty_months=12`, `payment_method="credit_card"`), modelled the same way Vivo's "Plano anual 12x no cartão" is (#26) so the Digital pick keeps reading the no-commitment Mensal. ⚠️ **The page itself labels EVERY Fit etiqueta — annual included — "sem prazo de permanência";** that wording is recorded verbatim in `price_note` rather than discarded, so the modelling choice stays visible and reversible.
  - **Card-vs-modal disagreement is REPORTED, not resolved:** SP's Mensal 1.0 card reads **R$20** while its own modal still reads **R$35** (the *demais-UFs* card and modal both say R$35). We record what the SP visitor is shown (the card) and flag the discrepancy in `price_note` — GOVERNANCE: never invent a number.

- **Vivo BRL parsing rule (#36, P0):** `vivo_convergent._price_from` used to strip every `.` as a thousands separator, so a **dot-decimal** `data-original-price="160.00"` became **16 000** — a silent 100× error feeding the convergent matrix. It now decides by pattern on the LAST separator: `1.200,00`/`24,99` → comma-decimal; `1,200.00`/`160.00` → dot-decimal; a lone separator with exactly 3 trailing digits (`1.200`) is a thousands group (BRL is never quoted with 3 decimals).
...
### Claro CROSS-SELLS a Flex plan on the Controle page — trust the slug, not the page (2026-08-05, CODE TASK #39) ← valuable IP

**Spotted by the Bridge reading the comparison sheet: "Claro Controle has the same price as Claro Flex — it's double counting Flex."** Correct, and here is the mechanism.

- **A Claro card's `category` and `plan_name` are inherited from the PAGE being parsed**, not from the card. `parse_next_data` derives the name as `f"{target.category_label} {gb}GB"` whenever the modal title is generic ("Mais detalhes"), and the category comes from the target. **Only the `plan_id` is genuinely Claro's** — it is the modal's accordion slug.
- **The controle page carries `plano-flex-20gb` at R$44,90.** The flex page carries the *same slug* at **R$59,90**. So one card, two pages, two prices — and on the controle page our parser christened it **"Controle 20GB"**.
- **Effect on the matrix:** R$44,90 undercut every genuine Controle plan (cheapest real one is **Controle 25GB R$59,90**), so **Control/claro read 44,90 — a Flex price — while Digital/claro also read 44,90** (Flex 15GB). The same money was counted in two columns and Claro's Controle line looked R$15 cheaper than it is.
- **Fix:** `slug_line()` maps Claro's own slug prefix to a product line (`plano-controle-*`→control, `plano-flex-*`→flex, `plano-pos-*`→postpaid). A card whose line disagrees with the page it was found on is **skipped and logged** — its own page remains authoritative, so the plan is still tracked, just once. An unrecognised slug shape is never filtered (fail open, not silent).
- **Why it started on 2026-08-05:** before #37 the dedupe key lacked `category`, so the collision hid the row entirely. #37 fixed the key and the miscategorised row surfaced — on **every snapshot since**. Swept all 49 snapshots: **6 affected dates, 2026-08-05 through 2026-08-10**, all removed. Each now reads Control **59,90** / Digital **44,90**, consistent with the 43 earlier dates.
- **The general lesson:** *a page is a place, not a fact.* Anything inherited from the page we happened to fetch — category, name, product line — is an assumption. Where the carrier states the fact itself (a slug, an etiqueta, a `field_regioes`), that is what to trust. Same principle as TIM's region gate (§3) and Claro Multi's `state=SP` parameter (§14).

### Vivo Controle's PRE-CHECKED add-on — the real source of the alert noise (2026-08-05, CODE TASK #37) ← valuable IP

- **Finding (audit P1 #3).** Every Vivo Controle card ships an **already-selected optional extra** — "+ 10 GB para suas redes sociais · R$ 5" — rendered as `[data-controller="switchUnique"]` with **`data-startdisabled="checked"`**. While it is checked, `.total-card-price-value` shows **base + add-on**, and the plan's own price sits in the element's **`data-original-price`**. We had been storing the inflated figure: **7 of 8** Controle headlines were wrong (80/75, 90/85 ×4, 95/90, 110/105).
- **Fix.** `_addon_base_price()` takes `data-original-price` as `price_brl` **only when** the switch is genuinely pre-checked **and** the attribute is *lower* than the displayed figure; otherwise the ordinary promo path runs unchanged. The add-on-inclusive figure is preserved in `price_note` ("a tela exibia R$ 80 com o adicional pré-selecionado (+R$ 5/mês, opcional)") — **context, never the headline**. Live SP 2026-08-05: **59 / 75 / 85 / 85 / 85 / 85 / 90 / 105**.
- **⚠️ The GB comes off WITH the price.** `.unique-card__header-benefit` reports the add-on-**inclusive** allowance: the unchecked card reads **46 GB @ R$59**, the checked ones **61 GB** over a base of R$75 — 61 = 51 + the add-on's 10. Storing the base price beside 61 GB would describe a package Vivo does not sell (61 GB costs R$80) and understate R$/GB by ~16 % on 7 of 8 plans. `_addon_gb()` reads the switch's own **`data-gb-bonus`** (base64: `"MTA="` → 10, alongside `data-price-bonus` `"NQ=="` → 5) and falls back to the "+ 10 GB" label; the subtraction only runs when both numbers are readable and the result stays positive. Stored now: **75/51 GB**, and 51 = 20 franquia + 31 bônus, matching `data_note` exactly. *(Caught by the #37 adversarial review — the first fix de-bundled the price but not the data.)*
- **⚠️ The `lite` branch is the MIRROR IMAGE and must stay separate.** For Vivo Easy Lite the **displayed** price is the no-commitment headline and `data-original-price` is the **12-month loyalty** price (#23/#26). Reading the attribute there would store a loyalty price and break the standing rule (§10.5). The category split is deliberate; a test pins both directions.
- **Why it mattered so much:** the add-on is *also* what made `data_gb` flip (61 GB with the extra, 51 GB without), and the old `plan_id` embedded that parsed GB — see below. **Measured over the 43 stored day-pairs: 79 of 79 Vivo-Controle alert lines are an exact ±R$5 move** — the add-on's fingerprint. That is 100 % of Vivo-Controle noise and **77 % of every alert line ever produced**.

### Stable Vivo mobile `plan_id` — no parsed GB, no price (2026-08-05, CODE TASK #37) ← valuable IP

- **Was:** `vivo:{VIV-code}-{data_gb}gb`. `data_gb` is a **parsed, volatile** number, so the same offer alternated between `…-61gb` and `…-51gb` as the add-on rendered — id-matching failed, the #29 name fallback re-paired the two variants, and a ±5 % line went out that reversed the next day.
- **Now:** `vivo:{VIV-code}` for control / lite / postpaid. The `VIV…` offer code is **unique per card** (verified live 2026-08-05: 8/3/11 distinct codes for 8/3/11 cards), so it stands alone. **Prepaid is the exception** — all its tiers share ONE page-level code, so the discriminator is the recarga **validity** (`vivo:VIV…-30d`), an *attribute*, never a price and never the add-on-skewed GB.
- **Evidence in the stored data:** `VIV202600030938` (Vivo Controle Netflix) is split across `…-61gb` (15 days @ R$95) and `…-51gb` (7 days @ R$90) — **one plan, two identities, two "prices"**. Under the #37 key it is one id at one price.
- **Honest limit:** A and B act at **capture** time. History is append-only, so stored rows keep their old ids and inflated prices — the flip only stops for snapshots taken from **2026-08-05** onward. A replay cannot demonstrate the improvement, and one that rewrites stored ids to "prove" it measures an artefact (attempted and discarded: a blanket suffix-strip also collapses Claro's *native* `…-25gb`/`…-30gb` slugs, which are genuinely different plans, manufacturing 540 fake moves).
...
#### Alerts extended to CONVERGENT + the fallback tightened (2026-08-05, CODE TASK #37)

- **The gap (audit P1 #6):** `alerts_from_workbook` hardcoded `sheet_name="history"` and the matcher read `plan_id`/`plan_name`, which combos don't have — so a Vivo Total combo moving R$160 → R$200 **e-mailed nothing**. Combos were tracked but unwatched.
- **Now:** the ≥3 % rule lives in one parameterised `compute_price_moves(..., id_attr, name_attr, domain)`; `compute_price_alerts` (mobile: `plan_id`/`plan_name`) and `compute_convergent_alerts` (combos: `offer_id`/`offer_name`) are thin wrappers. `alerts_from_workbook` reads **both** sheets and returns ONE list, sorted **mobile first, then |%| desc**; `format_alert_email` renders labelled **`MOBILE PLANS`** / **`CONVERGENT (COMBO) OFFERS`** sections and omits a section that has no lines. Same threshold, both directions, same graceful degradation (no password → skip; SMTP error → caught; **never blocks the scrape or the commit**).
- **Fallback TIGHTENED.** #29 only required the name to be unique among the **unmatched** rows, so a name that also existed on an id-matched row could still be claimed (the audit found **176 non-unique name groups** and a demonstrated false "Vivo Lite R$35 → R$75, +114 %"). It now pairs only when the name is unique in the **FULL snapshot on BOTH sides** — otherwise it refuses and the rows stay honest `new`/`removed`. **No match is better than a wrong match.** Re-keyed pairs print both ids in the digest: `[re-keyed tim:167041 → tim:180756]`.
- **⚠️ Convergent moves are only diffed when the combos were collected THIS run.** The convergent pass is fully guarded and legitimately yields nothing (a blocked carrier, a restructured page); `_merge_convergent` then adds no new `snapshot_date` — it preserves the old rows by design. Taking each sheet's own two latest dates would re-send yesterday's already-reported combo moves, **stamped with today's date, every day** until a fresh convergent snapshot lands. `alerts_from_workbook` therefore gates the convergent section on `conv_date == mob_date` and logs when it skips. *(Caught by the #37 adversarial review. A digest that repeats itself is a digest people stop reading.)*
- **Replay over the 43 stored day-pairs:** **19/43 days, 102 lines** (audit baseline 19/43, 109) — and the false Claro −25 % from the key coupling above is gone. That is the ceiling for what a *matching* change can do to data already written; the noise itself is a **capture** problem (the ±R$5 add-on, 77 % of all lines) that Parts A/B fix going forward.

#### SANITY GUARDRAIL — a degraded scrape must never be committed (2026-08-05, CODE TASK #37)

**The one rule to remember: a SANITY failure BLOCKS the commit; a price alert NEVER does.** Collection > notification, but *correct* collection > collection.

- **Config-driven bands** (`sanity:` in `config/sources.yaml` — no code change to tune). Mobile: claro 12–32, vivo 16–38, tim 12–30 (live 2026-08-05: **20 / 26 / 19**). Convergent: tim 2–10, vivo 6–20, claro 5–20 (live: **3 / 11 / 10**). Global price bounds **R$5–2000** — the ceiling is 2000, *not* 1000, because Vivo Total's "V 1 Giga" combo is genuinely **R$1.200/mo**; a tighter cap would band out real data every day.
- **`check_sanity(...) -> list[SanityIssue]`** is pure and judges **both** domains (`count_low` / `count_high` / `price_out_of_range`). Only carriers actually attempted are judged, so `--only tim` is not failed by the carriers it never tried.
- **Enforcement** sits in `main.py` **after** the convergent pass and **before** `write_workbook`: issues → log each one, send a `SANITY CHECK FAILED` e-mail (guarded — its failure cannot un-block anything), and **`return 4`**. The non-zero exit is what makes the workflow skip its commit step, so no degraded snapshot ever lands *and the good workbook on disk is never overwritten*.
- **The old zero-guard is subsumed**, and strictly improved: it used to return 3 *after* writing the workbook; zero is now just the extreme case of `count_low` and is caught *before* the write.
- **Verified live, not just in tests:** a mis-wired replay fed the wrong page to Claro and TIM; the guard fired `count_low` for both, exited 4, and left the committed workbook untouched.
- **Transient-only retry** (`sanity.fetch_retries`, default 2). `fetch_with_retry` + `is_transient_error` in `adapters/base.py` retry timeouts / connection resets / `net::ERR_*` with a short backoff, and re-raise a **block/4xx/CAPTCHA immediately** — GOVERNANCE §3 forbids retry-spamming a site that is refusing us. All **six** adapters now route through it; the previous per-adapter loops retried **any** `HTTPError`, i.e. they hit a 403 twice.

#### MISSED-RUN PROTECTION — the failure the guardrail cannot see (2026-08-05, CODE TASK #37)

The sanity guard only validates data **when the job runs**. A job that fails early, or never fires, is completely silent — which is exactly how 2026-08-04 was lost (see §8).

- **F2 — staleness alert.** At the **start** of every live run, `check_staleness()` compares the newest `history` snapshot with today; if older than `sanity.max_snapshot_age_days` (default **2**) it e-mails **`STALE DATA`** listing the age *and every missing calendar date*. Informational, fully guarded, never blocks — it is how the run that finally fires reports the days that were missed. (At the default of 2 days it deliberately tolerates a single skipped day; same-day detection is F3's job.)
- **F3 — watchdog workflow.** `.github/workflows/mobile-price-tracker-watchdog.yml`, daily **23:00 UTC = 20:00 BRT**, ~2 h after the scrape. It reads the **committed** workbook and e-mails **`NO SNAPSHOT TODAY`** if today's date is absent. **No scraping, no writing, no commit** — `permissions: contents: read`, its own concurrency group, a 10-minute timeout, and only `pandas/openpyxl/pyyaml` installed (no Playwright). It **always exits 0**: the watchdog reports, it never fails a workflow. A missing or unreadable workbook is itself worth an e-mail.
- Together: the guardrail protects the data the job collects, the staleness alert catches a *late* run, and the watchdog catches a run that **never happened**.

#### ⛔ A `--demo` run must never be committed (2026-08-05, CODE TASK #37)

`--demo` writes the adapters' **hardcoded sample prices** to the same workbook path, and **every output guard is skipped** in demo mode (both the staleness check and the sanity guardrail are `if not demo`). The main workflow exposes `mode: demo` as a dispatch input, so a demo dispatch would have committed **fabricated numbers into the append-only history as if they were real** — the exact thing GOVERNANCE forbids. The `Commit daily snapshot` and `Weekly backup` steps are now gated on `mode == 'live'`. Demo proves the pipeline; it is never data. *(Caught by the #37 adversarial review.)*
...
### A plan's identity includes its CATEGORY (2026-08-05, CODE TASK #37) ← valuable IP

`plan_id` is unique **within a carrier's category page**, not across the whole carrier — a fact three separate keys were getting wrong.

- **The bug (audit P1 #4).** `_merge_history` deduped on `(snapshot_date, carrier, state, _key)`, without `category`. A carrier's native id is only unique **within a category page**, so two rows sharing an id collided and one was silently dropped. Fixed by adding `category` to the dedupe key — the principle is right and stands.
- ⚠️ **But the row it "recovered" was NOT a Controle plan — see #39 below.** The audit read `claro:plano-flex-20gb` / "Controle 20GB" / R$44,90 as a fourth Claro Controle plan being lost. It is actually a **Flex plan cross-sold on the controle page**, and letting it into the `control` category made a Flex price win the Control column. The dedupe fix is correct; the row was miscategorised upstream of it, in the adapter.
- **⚠️ The coupling that made this subtle — found by the live replay, not by the tests.** Once *both* rows survive, one snapshot legitimately contains the **same `plan_id` twice**. The alert matcher and the `changes` sheet still keyed on `(carrier, state, plan_id)`, so the two collapsed into one row and the R$15 difference was reported as a **−25 % price cut that never happened**. `category` therefore had to go into **all three** keys at once: `_merge_history`, `_compute_changes` (`_ck`), and `alerts._key`. Fixing the dedupe without the matchers is *worse* than the original bug — it turns a missing row into a false alert.
- Convergent offers have **no** `category` attribute; `getattr` yields `None` consistently on both sides, so the shared matcher is unaffected. (Watch the tuple indices: the key grew from 3 to 4 elements — one positional read had to become `k[-1]`.)
- **The `changes` sheet and the alert must move together.** `_rotation_pairs` (excel_writer) and `compute_price_moves` (alerts) implement the *same* #29 rule; #37 first tightened only the latter, which would have had the sheet report a `price_change` the e-mail refused to send. Both now require full-snapshot name uniqueness. **Rule: never change one without the other** — §3 promises they agree, and a rule living in only one makes them contradict.
...
### THE LOST DAY — 2026-08-04, and what it teaches (diagnosed 2026-08-05, CODE TASK #37) ← valuable IP

- **What happened, from the run log (not inference).** The scheduled run **did fire** — `30955188476`, 2026-08-04T22:07:50Z — and **failed after 1m17s** in the **"Run tests (offline)" step**, which the workflow executes **BEFORE the scrape**:
  `tests/test_convergent_phase2.py::test_one_convergent_source_failing_does_not_stop_the_others_or_mobile — assert 3 == 0` · `1 failed, 144 passed`.
  The test (written in #32) stubbed **only `tim`**, so vivo and claro collected 0 plans and the then-committed `main.py` zero-guard returned **3** while the test asserted **0**. It passed locally only because an uncommitted work-in-progress had replaced that guard. The job never reached the scrape, so **no snapshot was written and nothing was committed**.
- **Not GitHub's fault:** the workflow was `active`, the cron fired on time, and every other night from 2026-07-18 to 2026-08-03 succeeded. **Last successful scheduled commit: 2026-08-03** (`30857241298`).
- **Complete gap list, as of 2026-08-05 — `history`: 44 snapshots over the 45 days from 2026-06-22, MISSING exactly `2026-08-04` (98 % coverage). `convergent_history`: 2 of 3 days, missing the same date.** No other gap exists in either sheet.
- **2026-08-04 is UNRECOVERABLE.** Carrier pages show only *current* prices; there is nothing to re-read. It is **never** backfilled — history stays append-only and honest about the hole.
- **2026-08-05 was NOT lost after all.** Its scheduled run died the same way, but the day was re-scraped live at 22:59 BRT — inside the same calendar day, so this is normal collection, not a backfill. It was initially misfiled as `2026-08-06` by the UTC-stamping bug above; that mislabel was corrected with the Bridge's approval.
- **THE LESSON, and it is a sharp one:** *a test that gates the scrape is a data-loss risk, not just a red X.* A CI test failure and a scrape failure have wildly different costs here, but the workflow treated them identically. **✅ FIXED STRUCTURALLY IN #38 — see the next section. And it very nearly cost TWO days:** the same stale test also killed the **2026-08-05** scheduled run (`31051578644`, `1 failed, 162 passed` — the pre-#37 tree, because the #37 push landed after 22:08 UTC). **2026-08-05 was saved only because it was re-scraped at 22:59 BRT, before the day ran out; 2026-08-04 was not, and never will be.** Detection was never going to be enough; the dependency itself had to go.

### CI: the test suite no longer gates data collection (2026-08-06, CODE TASK #38) ← the structural fix

**Two consecutive daily runs were killed by red unit tests that had nothing to do with scraping** (2026-08-04 and 2026-08-05). The workflow ran `pytest` *before* the scrape *in the same job*, so any failing test killed the run before it reached a carrier — and carrier pages only ever show today's price, so a missed day is gone for good. **08-05 was rescued by a manual re-scrape inside the same calendar day; 08-04 was not, and is permanently lost.** One day survived by luck and timing — that is not a control.

- **The fix: two INDEPENDENT jobs, `tests` and `collect`, with NO `needs:` between them.** They start together and neither can affect the other.
- **Why not `needs: [tests]` + `if: always()`** (the obvious alternative): that still makes collection *wait* for the suite, so a hung, queued or cancelled tests job delays or skips the scrape. The point is that collection has **no dependency on the suite at all** — not a weakened one.
- **Decoupling must not become silence.** The `tests` job still goes RED (GitHub's own notifications fire) **and** sends a **`CI TESTS FAILING`** e-mail over the same secret-based path as the price digest, `continue-on-error` so a mail problem cannot mask the real failure. The body states plainly that collection is unaffected, so a red test never reads as an emergency.
- **Least privilege:** `tests` runs with `contents: read`; only `collect` gets `contents: write`. `tests` also skips the Playwright browser download — no test launches a browser.
- **What still BLOCKS the commit, unchanged — a bad SCRAPE must never land:** nothing collected (exit 2), sanity bands / price bounds violated (exit 4, *before* the workbook is written), and `mode == demo`. **Verified empirically, not assumed:** with the tracker step forced to exit 4, `Commit daily snapshot` and `Weekly backup` were both **skipped** — a custom `if:` expression does *not* displace GitHub's implicit `success()`.
- **Proof the decoupling works** (also empirical): a scratch branch carrying one deliberately failing test produced `tests: failure` **and** `collect: success` in the same run, with all three carriers scraped and the snapshot committed. Both scratch branches were deleted; neither the failing test nor the forced exit ever touched `main`.
- **Residual risk, stated honestly:** a red suite now means untested code keeps collecting and committing. That is the deliberate trade — a wrong parser is fixable from the raw captures and the append-only history, a missing day is not.

#### `snapshot_date` is the SÃO PAULO day, not the runner's UTC day (2026-08-05, #38 follow-up) ← valuable IP

These are Brazilian prices, so a snapshot belongs to the **São Paulo** calendar day — `config/sources.yaml` has declared `timezone: America/Sao_Paulo` from day one. But `main.py` stamped `run_ts = datetime.now()`, which on a **UTC** CI runner is the UTC day. The daily cron (21:00 UTC == 18:00 BRT) lands on the same date in both zones, so **44 snapshots went by without exposing it**.

A manual run at **21:18 BRT on 2026-08-05** was already 00:18 UTC on the 6th, and that evening's São Paulo prices were filed as **`2026-08-06`** — tomorrow's date, for prices that no longer existed by then. It also meant `latest`, `changes` and both matrices' newest column pointed at a future-dated row.

- **Fix:** `main.project_now(settings)` returns `datetime.now(ZoneInfo(settings.timezone))` with the tzinfo **stripped**, so `snapshot_ts` keeps the exact naive format all 2 400+ stored rows use — the column always meant local wall clock. An unknown/missing tz name logs and falls back to `datetime.now()`: a clock detail must never be the reason a day's prices are lost.
- **`Settings.timezone`** reads `project.timezone`, defaulting to `America/Sao_Paulo`.
- **The one misdated snapshot was corrected**, with the Bridge's explicit approval: the 65 + 24 rows filed as `2026-08-06` were removed and the day re-scraped, landing correctly as **`2026-08-05`**. This was a *mislabel correction*, not a backfill — the prices were always genuine, only the date was wrong. ⚠️ Removing a date needs a **fresh output file**: `write_workbook` merges with what is on disk and `_merge_history` only replaces the dates present in the incoming set, so writing a filtered set over the existing workbook leaves the unwanted date exactly where it was (observed, then fixed by writing to a new path and `os.replace`-ing).
- **Consequence for the record:** the permanent gap is now **2026-08-04 only** (44/45 days, 98 %). 2026-08-05 is genuine, correctly-dated, freshly-scraped data.

#### The watchdog's date must be anchored to its SCHEDULED hour (2026-08-06, #38)

GitHub cron is best-effort. Observed the same night: the 23:00 UTC watchdog actually started at **00:02 UTC** — past midnight — so a naive `date.today()` asked "is there a snapshot for *tomorrow*?", which is unanswerable and would cry wolf on every slow queue. The workflow now evaluates the UTC date **as of two hours ago**, which is correct when the cron fires on time (23:00 − 2 h = same day) and stays correct for delays up to ~3 h. `watchdog.py` already accepted an explicit `today`, so this is a workflow-only change.

### ⚠️ `data/raw/` is the AUDIT TRAIL — a replay is a READER of it (2026-08-05, CODE TASK #37)

Twice in one session a capture was destroyed by tooling that had no business writing:
1. A replay harness patched `httpx.get` **per module** — but `tim.httpx`, `claro.httpx` and the convergent modules are the **same module object**, so the last patch won and every TIM target received the Ultracombo page. `_save_raw` then overwrote all four real TIM captures with it (four files, one MD5).
2. A review subagent exercising the live Vivo Total adapter got an error page and `_save_raw` replaced the 1.2 MB capture with **147 bytes**.

Both were repaired with the **minimum** targeted re-fetch (3 TIM pages — the `controle` page backs both `control` and `fit`; 1 Vivo render), and both were caught only because the sanity guardrail refused the degraded run. **Rules:** any replay/verification tool must stub `_save_raw`; only a real scrape writes to `data/raw/`; and a same-day capture set should be checksummed before it is trusted as evidence.

### §7 audit status after #37 (`docs/source_audit_2026-08.md`)

- 🔴 **P0 — both RESOLVED** in #36 (TIM Fit parser rebuild; Vivo convergent 100× price bug).
- 🟠 **P1 — all four RESOLVED** in #37: **#3** Vivo Controle add-on → base price (§3) · **#4** `category` in the dedupe key (§6) · **#5** stable Vivo `plan_id` (§3) · **#6** convergent alert coverage + tightened fallback (§5). *(#6's "new/removed lines in the digest" sub-item was deliberately not taken: new/removed are not price moves and adding them would re-introduce noise on every carrier id rotation — the `changes` sheet already carries them.)*
- 🟡 **P2 — items 7, 8, 9 remain OPEN and optional.** One of them, "don't retry 4xx", was resolved as a side effect of the #37 retry policy. The rest are quality-of-life, not correctness: Vivo convergent card-count assertion + `raw_ref` overwrite, Claro ghost-vs-failure separation + `precoCombo: null`, the `fs/ro` title artifact, TIM Controle's list price, and the convergent blank-cell footnote wording.
...
## 11. Changelog
- **0.13.0 — 2026-08-19** — CODE TASK #40 (**a blocked carrier must not cost the whole day**). On 2026-08-18 TIM returned **403 Forbidden to the GitHub runner on all five URLs** (verified same-day: the identical request from Rafael's residential IP returns 200, so TIM is refusing Azure/datacenter ranges, not us). `tim: 0` tripped `count_low`, the guardrail blocked the commit, and **Claro's 19 + Vivo's 26 perfectly good plans were discarded with it — 2026-08-18 is unrecoverable.** The guardrail was right to distrust `0`; it was wrong to treat *"never answered"* the same as *"answered with junk"*. **(1) UNAVAILABLE vs DEGRADED:** `main.run` now tracks, per carrier, targets attempted vs targets that failed to fetch; a carrier where **every** target raised is `unavailable` and is **excluded from the count bands** (`check_sanity(unavailable=…)`), so the carriers that answered still commit. A carrier that *answered* with an implausible count still blocks — the guardrail is not weakened, only made able to tell the two apart. The gap is reported by its own **`CARRIER UNAVAILABLE`** e-mail and left honest: never carried forward, never estimated. **(2) CONVERGENT CAN NO LONGER BLOCK MOBILE** — §14 has always promised this, but #37 broke it by routing convergent bands to the same blocking exit. `SanityIssue` now carries a `domain`; convergent issues drop *that run's* combo rows (the stored convergent history is preserved) and e-mail `CONVERGENT DATA SKIPPED`, while mobile commits. Found by the #40 end-to-end test, not by inspection. **(3) CATCH-UP PASS:** a second cron at 00:00 UTC (21:00 BRT, same São Paulo day) runs `--only-if-incomplete` — it scrapes **only** the carriers missing from today's snapshot and exits immediately when the day is complete, so a healthy day costs zero extra requests and no source is ever hit twice. A 403 is still never retried *inside* a run (GOVERNANCE §3); this is one second chance at a different hour, hours later. Tests 206 → **212**, including a full replay of 2026-08-18 through `main.run()` (mutation-checked: both new guards fail the suite when reverted).
- **0.12.2 — 2026-08-05** — CODE TASK #39 (Bridge-spotted). **Claro's Controle column was showing a FLEX price.** Claro cross-sells `plano-flex-20gb` on the **controle** page at **R$44,90** (the flex page prices the same slug at R$59,90), and because a card's `category`/`plan_name` are inherited from the page being parsed — only `plan_id` is genuinely Claro's — it was stored as **"Controle 20GB" R$44,90**. That undercut every real Controle plan (cheapest is **Controle 25GB R$59,90**), so **Control/claro and Digital/claro both read 44,90 — the same money counted in two columns.** New `slug_line()` maps Claro's own slug prefix to a product line and **skips + logs** any card whose line disagrees with the page it sits on; the line's own page stays authoritative, so nothing is lost, it is simply tracked once. An unrecognised slug shape is never filtered (fail open, not silent). **Affected every snapshot from 2026-08-05 on** (before #37 the category-less dedupe hid the row): **6 rows across 49 snapshots — 08-05, 08-06, 08-07, 08-08, 08-09, 08-10** — all removed; each date now reads **Control 59,90 / Digital 44,90**, matching the 43 earlier ones. Both matrices re-verified: baked == helper == live Excel recalculation, **609 cells, 0 mismatches**. Corrects the #37/§6 claim that this row was "a silently-dropped Claro Controle plan" — the dedupe fix was right, the row was miscategorised in the adapter. Tests 205 → **206**.
- **0.12.1 — 2026-08-05** — #38 follow-up (Bridge-requested). **`snapshot_date` is now the São Paulo day, not the runner's UTC day.** `config` has always declared `timezone: America/Sao_Paulo`, but `main.py` used `datetime.now()` — on a UTC CI runner that is the UTC day, and because the 21:00 UTC cron == 18:00 BRT the two agreed for **44 consecutive snapshots**. A manual run at 21:18 BRT on 08-05 was 00:18 UTC on the 6th and filed São Paulo's evening prices as **`2026-08-06`**, poisoning `latest`/`changes`/both matrices' newest column with a future-dated row. New `main.project_now(settings)` stamps in the project timezone and returns a **naive** datetime so `snapshot_ts` keeps the format all 2 400+ stored rows use; an unusable tz name logs and falls back rather than failing a scrape. The single misdated snapshot was removed and **2026-08-05 re-scraped live** (65 plans: claro 20 / tim 19 / vivo 26; 24 combos: tim 3 / vivo 11 / claro 10; sanity passed; both matrices 534 cells / 0 mismatches). ⚠️ Learned while doing it: **dropping a date requires writing to a FRESH file** — `write_workbook` merges with what is on disk and `_merge_history` only replaces the dates present in the incoming set. **The permanent gap is now 2026-08-04 alone (44/45 = 98 %).** Tests 203 → **205**.
- **0.12.0 — 2026-08-06** — CODE TASK #38 (**CLOSE-OUT**). *(Written before the 0.12.1 timezone fix — where this entry says 2026-08-05 was lost, it was in fact recovered a few hours later; see 0.12.1.)* **The test suite no longer gates data collection.** Two consecutive daily runs (2026-08-04 *and* 2026-08-05) were killed by the same stale unit test, because the workflow ran `pytest` before the scrape **in the same job** — a red test killed the run before it reached a carrier, and carrier pages only show today's price. Split into two **independent** jobs (`tests`, `collect`) with **no `needs:`**; `needs: + if: always()` was rejected because it still makes collection wait on the suite. A red suite stays **loud** — the job fails visibly *and* sends a `CI TESTS FAILING` e-mail (guarded) that says plainly the data is safe. Least privilege: `tests` is `contents: read`, only `collect` writes; `tests` skips the browser download. **Every data guard still blocks and was proven empirically:** with the tracker forced to exit 4, the commit and backup steps were **skipped** — a custom `if:` does not displace GitHub's implicit `success()`; and `mode: demo` still skips both. **The decoupling itself was proven, not assumed:** a scratch branch with one deliberately failing test produced `tests: failure` + `collect: success` in the same run, scraping all three carriers and committing the snapshot (both scratch branches deleted; nothing temporary reached `main`). Also fixed: the #32 convergent-guard test's isolation (it stubbed the whole `alerts_mod` with a class lacking `check_staleness`, so it passed by *accident* via a swallowed `AttributeError`, and still let the closing digest re-import the real module and read the real workbook + SMTP path — the stubs now patch the functions on the real module); and the **watchdog's date is anchored to its scheduled hour**, after its 23:00 UTC cron was observed firing at 00:02 UTC and asking about *tomorrow*. Live verification: 65 plans (claro 20 / tim 19 / vivo 26) + 24 combos (tim 3 / vivo 11 / claro 10), sanity passed, snapshot committed, both matrices baked == helper == formula across **534 cells, 0 mismatches**. **The staleness alert and the watchdog both fired for real on the genuine Aug 4–5 gap.** Tests **203**, all green. **The build phase is closed; the 🟡 P2 list stays open and explicitly optional.**
- **0.11.0 — 2026-08-05** — CODE TASK #37 (**FINAL build task — all four 🟠 P1 audit findings + unattended-operation hardening**). **DATA (§3/§6):** (1) **Vivo Controle stores the BASE price** — the "+10 GB · R$5" add-on ships **pre-checked**, so 7 of 8 headlines were inflated; live SP is now **59/75/85/85/85/85/90/105** with the on-screen figure kept in `price_note`. The mirrored `lite` rule (displayed = no-commitment, attribute = loyalty) is untouched and pinned by a test. (2) **Vivo `plan_id` is stable** — `vivo:{VIV-code}`, no parsed GB, no price; prepaid keeps a **validity** token. (3) **`category` joined the identity** in *all three* keys — `_merge_history`, `_compute_changes`, and `alerts._key` — recovering the silently-dropped **Claro Controle 20GB R$44,90** (control 3 → 4 rows) *without* the false −25 % the half-fix produced. **ALERTS (§5):** the ≥3 % rule now covers **convergent** offers too (`offer_id`/`offer_name`) in **one merged digest** with MOBILE / CONVERGENT sections, and the #29 name fallback **refuses** to pair a name that is non-unique in the full snapshot (re-keyed ids are printed). Replay: **19/43 days, 102 lines** (was 19/43, 109); **79 of 79** stored Vivo-Controle lines are an exact ±R$5 add-on artefact = **77 % of all noise ever**, removed at capture from here on. **GUARDRAIL (§5):** config bands per carrier for **both** domains + R$5–2000 price bounds; a failure **blocks the commit** (`return 4` **before** the write) and e-mails `SANITY CHECK FAILED`; the old zero-guard is subsumed and now fires *before* the workbook is written; **transient-only retry** across all six adapters (a 403/CAPTCHA is never retried — the old per-adapter loops retried any `HTTPError`). **MISSED RUN (§8):** 2026-08-04 diagnosed — the scheduled run fired and **died in the pre-scrape test step** (`assert 3 == 0` from a #32 test that stubbed one carrier); the day is **unrecoverable and is not backfilled**; it is the **only** gap in either sheet (44/45 days, 98 %). Added a **staleness alert** at run start and a **read-only watchdog workflow** (23:00 UTC, `contents: read`, no scraping, always exits 0). Both matrices re-verified end to end: history == helper == baked == **live Excel recalculation**, 534 cells. **An adversarial review of the diff (5 lenses, every finding independently refuted) confirmed 3 more real defects, all fixed here:** the add-on's **+10 GB had to come off with its R$5** (75/51 GB, not 75/61 — the row otherwise described an unbuyable package); `_rotation_pairs` had been left on the #29 rule while alerts.py was tightened, so the **`changes` sheet and the e-mail would have contradicted each other**; and the convergent digest section is now gated on the combos having been collected **this run**, so a guarded no-op pass can't re-send yesterday's moves daily. Also closed a latent hazard: **`mode: demo` could commit hardcoded sample prices** into the append-only history — the commit and backup steps are now gated on `live`. Tests 163 → **203**.
- **0.10.1 — 2026-08-05** — CODE TASK #36 (**P0 fixes from the #35 audit**; scope limited to the two blockers — the P1/P2 list stays open). **(1) TIM Fit parser rewritten** for TIM's ~2026-08-03 rebuild (§3): parsing is now anchored on each card's `data-modal-open` link, **region-gated** on the modal id's region list (the Fit section has no `field_regioes`), captures **all four SP tiers** instead of one-per-type, and restores the **stable etiqueta `plan_id`** (never the `tim:fit-*` slug). Live SP: `…306` Anual 1.0 25GB 12x R$20 · `…271` Anual 2.0 35GB 12x R$30 · `…270` Mensal 1.0 20GB R$20 · `…305` Mensal 2.0 35GB R$45. **The false alert no longer reproduces:** the plan tracked as "Fit Anual" (etiqueta 271) read R$30 on 2026-08-02 and reads **R$30** with the fixed parser — no move. The SP Mensal card-vs-modal contradiction (R$20 vs R$35) is **flagged in `price_note`**, not silently resolved. **(2) `vivo_convergent._price_from` 100× bug fixed** — separator roles decided by pattern; the 11 captured Vivo combos parse identically (R$160 → R$1 200). **First post-fix run emits NO spurious Fit alert** (verified live): the ids and names both change, so the #29 fallback cannot pair them — the `changes` sheet shows 4 new + 2 removed for tim/fit and new/removed are not alerted; the 3 alerts in the live run are the genuine TIM Controle cuts. History is append-only — the false 2026-08-03/04 R$20 rows stay, and are documented in `docs/source_audit_2026-08.md`. Two #28 tests that pinned the now-nonexistent old markup were **retired** (superseded by `tests/test_tim_fit.py`, built from the real rebuilt capture). Tests 145 → **163**.
- **0.10.0 — 2026-08-05** — CODE TASK #35 (**read-only full verification audit**; no parser/matrix/alert change). One polite SP pass over all 15 sources, every capture **re-parsed independently** and compared to what we store. Full note: **`docs/source_audit_2026-08.md`** (supersedes the #22 note, which predated #23/#24 and the whole convergent domain). **CONFIRMED CORRECT:** TIM prepaid validity 17/22/30 (#23); Vivo Lite no-commitment headline — the toggle SUCCEEDED, the 20GB card genuinely has no toggle (#26/#30); TIM postpaid `payment_method` on all 7 plans (#24); convergent counts/prices with **no drift** (TIM 3 · Vivo 11 · Claro 10 live + 5 ghosts); **both comparison matrices: history == formula == bake for EVERY cell** (Excel COM recalc, zero disagreements); alert plumbing (≥3 % rule, digest, secret-only-from-env, graceful degradation, never blocks the commit). **BLOCKER FOUND — the only price alert this project ever sent (2026-08-03 "TIM Fit −42.9 %/−33.3 %") was FALSE:** TIM restructured `#price-fit` into 2 regional blocks × 2 tiers, our first-match regex took a *different* ("1.0") offer, and `_fit_code`'s exact `modal-fit-anual/mensal` lookup broke (the ids are now `modal-fit-anual-1.0-sp-sc-rn-ce` …) so `plan_id` degraded `tim:TIM202600000271 → tim:fit-anual`; the tracked R$35 offer is still on the page and the R$20 Mensal is contradicted by its own modal. Also: **Vivo Controle headlines include a pre-checked R$5 add-on** on 7 of 8 cards (base prices live in `data-original-price`) — the root cause of the alert noise; **a Claro Controle card (R$44,90) is silently dropped** because `_merge_history` dedupes without `category`. **Alert coverage is MOBILE ONLY** — `convergent_history` is never read, so a combo price move sends no email; new/removed plans are also uncovered; replaying 43 day-pairs, 44 % of days would email and 79 % of lines are the Vivo render flip. **First adversarial review of the two phase-2 convergent adapters** (31 findings): a **100× price bug** in `vivo_convergent._price_from` (dot-decimal), a silently-short scroll, unstable `offer_id`s, and Claro's ghost rule absorbing catalog failures. Production status: committed `convergent_history` holds only **3 TIM rows** and `convergent_comparison` is **absent** from the committed workbook (#32–#34 landed after the last daily run). Prioritized fix list in §7 of the note. CI: **12/12 recent runs green**.
