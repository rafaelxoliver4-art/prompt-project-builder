...
- **THE LESSON, and it is a sharp one:** *a test that gates the scrape is a data-loss risk, not just a red X.* A CI test failure and a scrape failure have wildly different costs here, but the workflow treated them identically. **✅ FIXED STRUCTURALLY IN #38 — see the next section. And the lesson cost TWO days, not one:** the very same stale test also killed the **2026-08-05** scheduled run (`31051578644`, `1 failed, 162 passed` — the pre-#37 tree, because the #37 push landed after 22:08 UTC). Detection was never going to be enough; the dependency itself had to go.

### CI: the test suite no longer gates data collection (2026-08-06, CODE TASK #38) ← the structural fix

**Two calendar days of prices were lost to red unit tests that had nothing to do with scraping.** The workflow ran `pytest` *before* the scrape *in the same job*, so any failing test killed the run before it reached a carrier — and carrier pages only ever show today's price, so the loss is permanent.
